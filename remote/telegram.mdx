...
Send `/fast` to toggle faster replies for the current Telegram conversation. Fast mode turns reasoning off for quick, lighter work and can use fewer tokens. Send `/fast` again to return to High mode.

This is session-specific, not an account setting. `/newchat` starts a fresh conversation back in High mode.
...
| `/fast` | Toggle Fast mode for the current conversation. `/newchat` resets it to High. |
