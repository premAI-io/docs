...
Send `/fast` to toggle faster replies. Fast mode turns reasoning off for quick, lighter work and can use fewer tokens. Send `/fast` again to return to Default mode.

This is saved to your Fluso account. If you change it in Telegram, **Settings → Model mode** in the Mac app changes too, and the reverse is also true. See [Model mode](/features/chat#model-mode) for the desktop setting.
...
| `/fast` | Toggle faster replies for quick, lighter work. |
