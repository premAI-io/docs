...
- Kimi K3 becomes the default model in standard chats, and Confidential Mode keeps its own model on the encrypted path.
- Share a chat as a public read-only link.
- Move in and out of Confidential Mode whenever you want.

**Permissions built for teams**

- Most organisations want agents doing real work, and are wary of letting them loose on company systems.
- Fluso makes that a setting rather than a leap of faith.
...
- Every connected app now runs through [MCP](https://docs.fluso.ai/features/mcp).
- Gmail, Slack, Drive, the managed servers we host, any hosted server you already use, and anything you run yourself: they all connect the same way.
...
- A message that happens to say "thread" or "channel" is no longer mistaken for a Slack request.
- Web search is now called what it is.

**Fluso on iPhone**

- Fluso is now on the [iOS App Store](https://apps.apple.com/in/app/fluso/id6780937477).
- Fluso accumulates your context: your projects, your apps, and the way you like work done.
- Until now all of that lived on the desktop, which does little for you when you are walking out of a meeting.
- Ask for something by voice while you walk, or snap a photo of a whiteboard and send it straight in.
- Check what is overdue between meetings, or pick a project thread back up where you left it.
...
- Confidential Mode runs a separate model on the encrypted path, GLM 5.2 in this release.
...
- Excel and CSV files render as real spreadsheets in the viewer, with cell colours, merged cells and column widths kept.
...
- After sleep or a network change the app reconnects itself, instead of sitting on a dead connection.
- It stops mistaking a lost connection for scheduled maintenance.
- You stay signed in through brief authentication hiccups.
- A session now lasts 30 days of inactivity instead of 7.
...
- Onboarding's Terms and Privacy links go where they should.
- The app now uses your system font.
