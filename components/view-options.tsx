...
/**
 * Open the Fluso desktop app via its fluso:// URL scheme. If nothing handles
 * the scheme (app not installed), fall back to https://fluso.ai/ — detected by
 * the page still being visible shortly after the attempt.
 */
function openFluso() {
  const fallback = window.setTimeout(() => {
    window.location.href = 'https://fluso.ai/';
  }, 1500);
...
function Item({
  ...
  return (
    <button type="button" onClick={onClick} className={cn(itemClass, 'text-start')}>
      {icon}
      {title}
      <ExternalLinkIcon className="text-fd-muted-foreground size-3.5 ms-auto" />
    </button>
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="flex flex-col">
        ...
        />
        <Item title="View as Markdown" href={markdownUrl} icon={<TextIcon />} />
        <Item
