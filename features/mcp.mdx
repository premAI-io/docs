...
When an MCP tool needs confirmation, Fluso shows its server, tool name, and arguments before it runs. You can allow the exact request once, save an allow rule for that tool, or deny the request. Manage per-tool **Allow**, **Ask**, and **Deny** rules from the server's row under **Plugins**.

See [Approvals and permissions](/features/approvals) for approval modes, card behavior, and rule precedence.
