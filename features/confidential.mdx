...
description: Run your work through models hosted in a hardware-sealed enclave, encrypted with a key only you hold. What confidential compute is, how your data stays protected, and how to turn it on.
---
...
Your data is encrypted on your device before it leaves, so the network only ever carries scrambled bytes. Files and keys are stored encrypted, and no plaintext touches Prem's storage. The piece most systems leave exposed is data in use, the moment a model actually reads your request. Confidential mode closes that gap: because the model runs inside the TEE, your data stays sealed even while it's being processed. It never exists in plaintext outside your device and the sealed enclave.
